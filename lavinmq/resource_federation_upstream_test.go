...
func TestAccFederationUpstream_LocalCluster(t *testing.T) {
	t.Parallel()
	resourceName := "lavinmq_federation_upstream.test_local"
	exchangeDataSourceName := "data.lavinmq_exchanges.all_exchanges"
	...
	lavinMQResourceTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
					resource "lavinmq_exchange" "upstream_exchange" {
							name        = "test_fed_upstream_local"
							vhost       = "/"
							type        = "topic"
							auto_delete = false
							durable     = true
					...
					resource "lavinmq_exchange" "downstream_exchange" {
							name        = "test_fed_downstream_local"
							vhost       = "/"
							type        = "topic"
							auto_delete = false
							durable     = true
					...
					resource "lavinmq_federation_upstream" "test_local" {
							name     = "vcr_test_local_federation"
							vhost    = "/"
							uri      = "%[1]s"
							exchange = lavinmq_exchange.upstream_exchange.name
					}

					resource "lavinmq_policy" "federation_policy" {
							name       = "test_fed_local_policy"
							vhost      = "/"
							pattern    = "^test_fed_downstream_.*"
							apply_to   = "exchanges"
							priority   = 10
							definition = {
									"federation-upstream" = lavinmq_federation_upstream.test_local.name
							}

							depends_on = [
									lavinmq_exchange.upstream_exchange,
									lavinmq_exchange.downstream_exchange,
									lavinmq_federation_upstream.test_local
							]
					}
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", "vcr_test_local_federation"),
					resource.TestCheckResourceAttr(resourceName, "exchange", "test_fed_upstream_local"),
					resource.TestCheckResourceAttrSet(exchangeDataSourceName, "exchanges.#"),
					resource.TestCheckTypeSetElemNestedAttrs(exchangeDataSourceName, "exchanges.*", map[string]string{
						"name":                      "test_fed_upstream_local",
						"message_stats.publish_in":  "0",
			...
			},
			{
				Config: fmt.Sprintf(`
					...
					resource "lavinmq_publish_message" "test_message" {
							vhost       = "/"
							exchange    = lavinmq_exchange.upstream_exchange.name
							routing_key = "test.local.federation"
							payload     = "{\"message\": \"VCR test local federation\"}"
							properties = {
									content_type = "application/json"
							}
